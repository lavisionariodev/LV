-- Extend create_checkout_from_cart() to store buyer-provided deceased details and wake duration.
-- Supports product_id with optional ::pkg:: suffix.
...
begin
  ...
  select exists (
    select 1
    from public.cart_items ci
    left join public.seller_listings sl
      on sl.id::text = split_part(ci.product_id, '::pkg::', 1)
    where ci.user_id = p_buyer_id
      and (p_product_ids is null or ci.product_id = any(p_product_ids))
      and (
        sl.id is null
  ...
  -- wake_duration_days: optional; treat blank/non-numeric as null.
  if nullif(btrim(coalesce(p_contact->>'wake_duration_days', '')), '') is not null then
    begin
      v_wake_days := (btrim(p_contact->>'wake_duration_days'))::int;
  ...
  inserted_orders as (
    ...
    select
      ...
      nullif(btrim(coalesce(p_contact->>'service_location', '')), ''),
      nullif(btrim(coalesce(p_contact->>'deceased_name', '')), ''),
      nullif(btrim(coalesce(p_contact->>'date_of_death', '')), '')::date,
      case when v_wake_days is not null and v_wake_days >= 0 then v_wake_days else null end
    from per_seller ps
    returning *
  ),
  inserted_items as (
