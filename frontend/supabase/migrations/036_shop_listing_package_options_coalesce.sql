  -- Shop package dropdown: prefer per-listing dynamic_values.package_options (string_list / JSON array),
  -- fall back to sellers.package_options when the listing has none (seller settings / legacy).
  ...
  comment on function public.get_active_shop_listings() is
    ...
    'RLS on sellers/seller_listings. seller_package_options prefers listing dynamic_values.package_options '
    'when non-empty, else sellers.package_options. '
