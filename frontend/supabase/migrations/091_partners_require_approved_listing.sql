-- Directory + spotlight: only sellers who have at least one buyer-visible service listing
-- (same gates as get_active_shop_listings: active seller, approved + active listing).
...
as $$
  ...
  where s.status = 'active'
    and exists (
      ...
      where sl.seller_user_id = s.user_id
...
comment on function public.get_active_partners_directory() is
  'SECURITY DEFINER: /partners + homepage — active sellers with ≥1 approved active listing; '
  'cover_photo_url, specialties, address, tenure, review aggregates.';
...
comment on function public.get_partners_spotlight() is
  'SECURITY DEFINER: /partners spotlight — eligible sellers have ≥1 approved active listing; '
  'featured + top_rated; featured rows include avg_rating.';
