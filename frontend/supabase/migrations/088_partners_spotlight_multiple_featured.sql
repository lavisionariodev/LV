-- Spotlight: return all admin-featured sellers (not only one). top_rated unchanged (already multi-row on ties).
...
comment on function public.get_partners_spotlight() is
  'SECURITY DEFINER: /partners spotlight — all partners_featured sellers + top_rated (tie rules on avg then count).';
