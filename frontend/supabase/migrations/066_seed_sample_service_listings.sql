-- Sample service listings for demo sellers: five distinct services per seller when missing.
-- Idempotent: skips a row when the same seller already has that exact listing_name.
-- Mirrors seller UI requirements (SellerListingForm.findFirstMissingRequiredField):
--   listing_name, base_price, image_urls (≥1 https URL), inclusions, who_this_is_for,
--   important_notes, funeral_category (non-other slugs here), description, listing_kind, etc.
--
-- Images: uses stable placeholder URLs (picsum.photos) so `<img>` works without Storage uploads.
-- For real listing-images bucket URLs, see frontend/docs/sellers.md § Listing images.

with sellers_seed as (
  select *
  from (
    values
...
-- Five different catalog services (same menu per shop; copy varies by seller name in description).
service_catalog as (
  select *
  from (
    values
