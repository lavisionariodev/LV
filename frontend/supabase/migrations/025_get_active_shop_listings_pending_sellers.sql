-- Shop RPC: include listings from sellers who are not yet "active".
-- New seller rows default to status = 'pending' (012_create_sellers_table.sql); requiring
-- s.status = 'active' meant zero rows for typical dev/OJT flows until an admin flips the seller.
-- Tighten back to s.status = 'active' only if you want the marketplace hidden until approval.
