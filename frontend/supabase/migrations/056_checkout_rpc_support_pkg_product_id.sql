-- Support cart_items.product_id values that include package suffix:
--   <listing_uuid>::pkg::<urlencoded_package>
...
begin
  ...
  with selected_cart as (
    select
      ci.product_id,
      split_part(ci.product_id, '::pkg::', 1) as listing_id_text,
      nullif(ci.product_id, split_part(ci.product_id, '::pkg::', 1)) as product_variant,
      ci.quantity,
      sl.seller_user_id,
      sl.listing_name,
      sl.base_price
