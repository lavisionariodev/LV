-- ============================================================================
-- Demo account — Evercare Life Services (five sample service listings)
-- Same catalog as migration 066.
--
-- Credentials (reference only; passwords belong in Auth / secrets, not in SQL):
--   Shop name     Evercare Life Services
--   Demo email    s2020105028@firstasia.edu.ph
--   sellers.user_id / auth.users.id
--                 6da71112-f1c9-4d94-b7eb-16a3a762a566
--
-- Use when 066 inserted nothing for this seller (e.g. inner join to public.sellers failed).
--
-- Preconditions:
--   • auth.users includes the UUID above (seller can sign in).
--   • public.sellers has a row with user_id = that UUID (FK on seller_listings.seller_user_id).
--
-- Idempotent: skips when the same seller_user_id + listing_name already exists.
-- ============================================================================
